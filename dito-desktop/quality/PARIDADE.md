...
| A7 | Ditado longo **não estoura 2 GiB** | comentário do Orca (#7925) | janela de 20 s no `engine-worker.js` — **código feito, não exercitado** | PENDENTE (W2) |
...
| B2 | F9 continua funcionando depois da 1ª vez | `hotkey_repeat_test` (armadilha real) | 5 ciclos seguidos no roteiro | PENDENTE |
| B3 | Tecla suprimida não fica "presa" | `hotkey_stuck_test` | estado físico + 300 ms contínuos | PENDENTE |
| B4 | Modo alternar (padrão) e modo segurar | decisão do dono | implementado; alternar exercitado, segurar não | PENDENTE |
...
| # | Comportamento | Vem de | Quem prova | Estado |
|---|---|---|---|---|
| C1 | Cola no Bloco de Notas (controle) | `paste_sequence_test` | `sonda_colagem.ps1` alvo `notepad` | reusável |
...
| C4 | Texto longo chega **antes** do Enter | sonda já mede | mesma sonda | reusável |
...
| E4 | Cartão de revisão | `review_card_test` | teste de componente | PENDENTE |
