...
bool Win32Window::Create(const std::wstring& title,
  ...
  // Fork: no DWM alpha here — Flutter's release swapchain composes opaque and blur-behind
  // rendered the whole window invisible. Overlay shape comes from SetWindowRgn instead.
...
bool Win32Window::Show() {
  if (!focusable_) {
    // Fork: SWP_SHOWWINDOW|SWP_NOACTIVATE shows without ever taking the foreground.
...
void Win32Window::SetChildContent(HWND content) {
  ...
  // Fork: this SetFocus is what pulled the foreground away on every HUD appearance.
  if (focusable_) {
    SetFocus(child_content_);
  }
}
