...
if (!bound || !bound.includes('installed=true')) await finish(1, 'HOTKEY: FALHA - o hook nao instalou')

tapKey()
if (!(await waitFor(pid, 'dictation: started', 8))) {
  await finish(1, 'HOTKEY: FALHA - apertei ' + key + ' e o ditado NAO comecou (o evento nao chegou ao app)')
}
...
if (!(await waitFor(pid, 'dictation: stopped', 8))) {
  await finish(1, 'HOTKEY: FALHA - apertei ' + key + ' de novo e o ditado NAO parou')
}
...
if (!(await waitFor(pid, 'transcribed:', 30))) {
  await finish(1, 'HOTKEY: FALHA - o audio nunca chegou ao motor')
}
