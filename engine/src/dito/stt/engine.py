...
def model_cached(name: str) -> bool:
    ...
    for cache_path in MODEL_CACHES:
        if not cache_path or not cache_path.exists():
            continue
        for folder in cache_path.glob(f"models--*faster-whisper-{name}"):
            # The weights live in blobs/; a snapshot with no big blob behind it is a half download.
            blobs = folder / "blobs"
            if blobs.exists() and any(f.stat().st_size > 10_000_000 for f in blobs.glob("*") if f.is_file()):
                return True
    return False
